...
<body>
    <content tag="column1">
        <g:render template="ordersTemplate" model="[orders: orders, processId: processId, ids: ids]"/>
    </content>

    <content tag="column2">
        <g:if test="${order}">
            <g:render template="show" model="[
                    order: order, user: user, currencies: currencies,
                    periodUnit: periodUnit, isCurrentCompanyOwning: isCurrentCompanyOwning]"/>
        </g:if>
    </content>
</body>
</html>
