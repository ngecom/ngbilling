...
$(document).ready(function () {
    $(jqTableGrid).jqGrid({
        url:'<g:createLink action="findPreferences"/>',
        datatype: "json",
        colNames:[
            '<g:message code="preference.th.type"/>',
            '<g:message code="preference.th.value"/>'
        ],
        colModel:[
            { name: 'preferenceId', editable: false, width: 150, formatter: preferenceFormatter },
            { name: 'value', editable: false, sortable: false, search: false }
        ],
        sortname: 'preferenceId',
        sortorder: 'desc',
        autowidth: true,
        height: 'auto',
        rowNum: 50,
...
function preferenceFormatter (cellvalue, options, rowObject) {
    var preferenceIdDisplay = '<em><g:message code="table.id.format" args="['_preferenceId_']"/></em>'
    var content = '<div class="medium">' + rowObject.description + '</div>' + preferenceIdDisplay;
    return content.replace(/_preferenceId_/g, cellvalue)
}
