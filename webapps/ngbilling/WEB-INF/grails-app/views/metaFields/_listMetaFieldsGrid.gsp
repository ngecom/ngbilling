...
$(document).ready(function () {
    $(jqTableGrid${updateColumn}).jqGrid({
        url:'<g:createLink action="findMetaFields" id="${selectedCategory}"/>',
        datatype: "json",
        colNames:[
            '<g:message code="metaField.th.name"/>'
        ],
        colModel:[
            { name: 'metaFieldId', editable: false, search: false, sortable: false, formatter: metaFieldFormatter }
        ],
        sortname: '',
        sortorder: 'desc',
        autowidth: true,
...
function metaFieldFormatter (cellvalue, options, rowObject) {
    ...
    var content = '<div class="medium">' + rowObject.name + '</div>' + metaFieldIdDisplay;
